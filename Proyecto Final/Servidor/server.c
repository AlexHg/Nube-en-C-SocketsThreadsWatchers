...
void *dist_request(){
    char file_request[1024];
    char file_request_prefix[1024];
    int request_mode;
    int meta_clientes = 3;
    int count, clients[meta_clientes];
    char buffer[TRAMAS];
    int dist_file;
    int recvBytes, breakit;

    char dist_path_prefix[] = "dist-";//realpath -> dist-cloud/
    char nofound[] = "nofound";
    char brk[] = "BREAKED";

    RESTART_REQUEST:
        count = 0;
        scanf("%d", &request_mode);
        distr_request = 0;
        if(request_mode == 1){
            distr_request = 1;
            printf("MODO DISTRIBUIDO\n<<Recuerde que no podrá escuchar cambios en los clientes>>\nPorfavor ingrese el archivo requerido en la petición distribuida: ");
            scanf("%s", file_request);

            printf("Buscando clientes (%d) conectados...\n\n", meta_clientes);

            for(int i = 0; i < NUMERO_CLIENTES; i++){
                if(count >= meta_clientes) break;
                if(conections[i] == 1){
                    clients[count] = i;
                    count++;
                }
            ...
            if(count < meta_clientes-1){
                printf("No se encontraron clientes suficientes para la petición distribuida\nRegresando al modo de espera\n");
                goto RESTART_REQUEST;
            }

            printf("Se encontraron los siguientes clientes conectados:\n");
            ...
            printf("Iniciando peticiones\n");
            //Enviar petición
            int progress = 0;
            char file_request_client[1024];
            strcpy(file_request_client, "cloud/"); // cloud/
            strcat(file_request_client, file_request); // cloud/<nombreArchivo>

            strcpy(file_request_prefix, dist_path_prefix); // dist-
            strcat(file_request_prefix, file_request_client); // dist-cloud/<nombreArchivo>

            dist_file = open(file_request_prefix, O_WRONLY|O_CREAT, 0700);
            for(int j = 0; j < count; j++){
                printf("Enviando petición a Cliente %d\nRequiriendo parte %d de %s\n", clients[j], j, file_request);

                //ENVIAR NOMBRE DE ARCHIVO CON EL NUMERO DE "PARTE" QUE NECESITA ENVIAR
                char requestString[1024];
                sprintf(requestString, "%d,", j);
                strcat(requestString, file_request_client);
                strcat(requestString, ",distribute,true");
                if(send(idc[clients[j]], (void *)requestString, sizeof(requestString), 0)<0){
                    printf("\nerror al enviar\n");
                    exit(1);
                }

                //RECIBIENDO LA PARTE <j> DE PARTE DEL CLIENTE <clients[j]>
                while( (recvBytes = recv(idc[clients[j]], (void *)buffer, sizeof(buffer), 0)) > 0 ){
                    
                    //SI NO EXISTE EL ARCHIVO EN EL CLIENTE RETORNARÁ EN EL BUFFER <"nofound"> 
                    if(strcmp(buffer, nofound) == 0){
                        printf("No se encuentró el archivo %s\n",file_request);
                        printf("\nSALIENDO DEL MODO DISTRIBUIDO\n<<Los cambios en los clientes ya serán escuchados a partir de ahora>>\nRegresando al modo de espera\n\n");
                        goto RESTART_REQUEST;
                    }                            

                    //SI EL CLIENTE ENVÍA LA PALABRA RESERVADA <"BREAKED"> SIGNIFICA QUE TERMINÓ DE ENVIAR SU PARTE                            
                    ...
                    progress += TRAMAS;
                    printf("[DESCARGANDO (%d/%d) DE CLIENTE %d %s] \t%dKB \n",j+1,meta_clientes,clients[j],file_request, progress);
        

                    if(write(dist_file, &buffer, recvBytes)<0){
                        printf("\nerror en el write\n");
                    }
...
int main(int arg0, char *arg1[]){
    ...
    pthread_t peticion_distribuida;
    for (int i = 0; i < NUMERO_CLIENTES; i++){
    ...
    sleep(1);
    pthread_create(&peticion_distribuida, NULL, dist_request, NULL);
    for (int i = 0; i < NUMERO_CLIENTES; i++){
